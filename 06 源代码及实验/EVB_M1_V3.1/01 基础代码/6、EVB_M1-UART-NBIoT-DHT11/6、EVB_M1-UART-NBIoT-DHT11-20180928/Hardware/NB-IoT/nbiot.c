...
uint8_t NB_ReceiveMsg(uint8_t *msgReceive)
{
	...
	pos3 = strstr((char *)USART2_RX_BUF,"+NSONMI:0");
	if(pos3)
	{
		HAL_Delay(500);
		char *recvBuf;
		...
		USART2_RX_LEN = atoi(pos3+10);				//获取要接收数据的长度
		printf("USART2_RX_BUF_LEN: %d \r\n",USART2_RX_LEN);
		recvBuf =(char *)malloc(sizeof(uint8_t)*USART2_RX_LEN*2);
		memset(recvBuf,0,USART2_RX_LEN*2);
		DecToString(USART2_RX_LEN,&msg[strlen(msg)]);//AT指令拼接
		strcat(msg,"\r\n");
		memset(USART2_RX_BUF,0,strlen((const char *)USART2_RX_BUF));
		USART2_RX_LEN = 0;
		HAL_UART_Transmit(&huart2, (uint8_t *)msg, strlen((const char *)msg), DefaultTimeout);//发送取数据指令
		while(NULL!=strstr((const char *)USART2_RX_BUF,",5683,"));//接收到数据
		HAL_Delay(200);								//为了能够完全接收到所有数据
		printf("NB DATA RECEIVE: %s \r\n",USART2_RX_BUF);
		recvBuf = strtok((char *)USART2_RX_BUF,",");//AT+NSORF=0	
...
//		printf("message长度: %s \r\n",recvBuf);
		...
		memcpy(msgReceive,recvBuf,USART2_RX_LEN*2);
		free(recvBuf);
		recvBuf = NULL;
		memset(USART2_RX_BUF,0,USART2_RX_LEN);
		USART2_RX_LEN = 0;
