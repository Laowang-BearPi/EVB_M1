...
import com.mcz.smoke_appproject.app.ui.activity.InputManualActivity;
import com.mcz.smoke_appproject.app.ui.zxing.zxing.new_CaptureActivity;
                ...
                if (!servicesinfo.equals("null")){
                    ...
                    for (int j = 0; j < jsa.length(); j++) {
                      ...
                      dataInfo.setDeviceCSQ(jsonObject.optString("CSQ"));
                        dataInfo.setDeviceSmokeValue(jsonObject.optString("SmokeValue"));
                        dataInfo.setDeviceMaxValue(jsonObject.optString("MaxValue"));
...
//
                  ...
                  dataInfo.setDeviceCSQ("暂无数据");
                    dataInfo.setDeviceSmokeValue("暂无数据");
                    dataInfo.setDeviceMaxValue("暂无数据");
    ...
    private void addlistdata(List<DataInfo> result) {
        ...
        if(result.size()>0){
            ...
            for (int j = 0; j < count; j++) {
                String YZ= result.get(j).getDeviceMaxValue();
                String YG= result.get(j).getDeviceSmokeValue();
                if (pan(YG)==true&&pan(YZ)==true)
                {
                    if (YG.contains("暂无数据") || YZ.contains("暂无数据") && mediaPlayer !=null){
                        try {
                            mediaPlayer.stop();
                            mediaPlayer.prepare();
                            mediaPlayer.seekTo(0);
                        ...
                        if (sp.getBoolean("isplayer",true)){
                            if (Double.parseDouble(YG)>=Double.parseDouble(YZ)){
                                if (!mediaPlayer.isPlaying()){
                                    mediaPlayer.start();
                                    playerIS=false;
                                    sp.edit().putBoolean("isplayer",playerIS).commit();
//                                sp.edit().putBoolean("isokb",true).commit();
                                }
